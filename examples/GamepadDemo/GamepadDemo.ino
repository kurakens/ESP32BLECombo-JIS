...
void setup() {
  ...
  cfg.mode = ESP32BLEComboJISMode::GAMEPAD_ONLY;
  cfg.deviceName = "ESP32 Gamepad";
  cfg.manufacturer = "Espressif";
  cfg.batteryLevel = 80;
  cfg.appearance = ESP32BLEComboJISAppearance::GAMEPAD;
...
void loop() {
  ...
  else if (line == "hat up") ble.gamepadSetHat(ESP32BLEComboJIS::HAT_UP);
  else if (line == "hat down") ble.gamepadSetHat(ESP32BLEComboJIS::HAT_DOWN);
  else if (line == "hat left") ble.gamepadSetHat(ESP32BLEComboJIS::HAT_LEFT);
  else if (line == "hat right") ble.gamepadSetHat(ESP32BLEComboJIS::HAT_RIGHT);
  else if (line == "hat center") ble.gamepadSetHat(ESP32BLEComboJIS::HAT_CENTERED);
