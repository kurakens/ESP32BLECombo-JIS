...
ESP32BLEComboJIS::ESP32BLEComboJIS()
    : m_connected(false), m_started(false), m_mode(ESP32BLEComboJISMode::KEYBOARD_MOUSE),
...
void ESP32BLEComboJISServerCallbacks::onConnect(NimBLEServer* pServer, NimBLEConnInfo& connInfo) {
    (void)pServer; (void)connInfo; m_owner->handleConnected(true);
}

void ESP32BLEComboJISServerCallbacks::onDisconnect(NimBLEServer* pServer, NimBLEConnInfo& connInfo, int reason) {
    (void)connInfo; (void)reason; m_owner->handleConnected(false); pServer->advertiseOnDisconnect(true);
}

void ESP32BLEComboJIS::handleConnected(bool connected) { m_connected = connected; }

bool ESP32BLEComboJIS::keyboardEnabled() const {
    return m_mode == ESP32BLEComboJISMode::KEYBOARD_ONLY || m_mode == ESP32BLEComboJISMode::KEYBOARD_MOUSE || m_mode == ESP32BLEComboJISMode::KEYBOARD_MOUSE_GAMEPAD;
}

bool ESP32BLEComboJIS::mouseEnabled() const {
    return m_mode == ESP32BLEComboJISMode::MOUSE_ONLY || m_mode == ESP32BLEComboJISMode::KEYBOARD_MOUSE || m_mode == ESP32BLEComboJISMode::KEYBOARD_MOUSE_GAMEPAD;
}

bool ESP32BLEComboJIS::gamepadEnabled() const {
    return m_mode == ESP32BLEComboJISMode::GAMEPAD_ONLY || m_mode == ESP32BLEComboJISMode::KEYBOARD_MOUSE_GAMEPAD;
}

bool ESP32BLEComboJIS::comboAllEnabled() const { return m_mode == ESP32BLEComboJISMode::KEYBOARD_MOUSE_GAMEPAD; }

void ESP32BLEComboJIS::resetReports() {
...
const uint8_t* ESP32BLEComboJIS::keyboardReportMap(size_t& len) { len = sizeof(kKeyboardMap); return kKeyboardMap; }
const uint8_t* ESP32BLEComboJIS::mouseReportMap(size_t& len) { len = sizeof(kMouseMap); return kMouseMap; }
const uint8_t* ESP32BLEComboJIS::comboReportMap(size_t& len) { len = sizeof(kComboMap); return kComboMap; }
const uint8_t* ESP32BLEComboJIS::gamepadReportMap(size_t& len) { len = sizeof(kGamepadMap); return kGamepadMap; }

uint16_t ESP32BLEComboJIS::resolveAppearance() const {
    if (m_config.appearance != ESP32BLEComboJISAppearance::AUTO) return static_cast<uint16_t>(m_config.appearance);
    switch (m_mode) {
        case ESP32BLEComboJISMode::KEYBOARD_ONLY: return static_cast<uint16_t>(ESP32BLEComboJISAppearance::KEYBOARD);
        case ESP32BLEComboJISMode::MOUSE_ONLY: return static_cast<uint16_t>(ESP32BLEComboJISAppearance::MOUSE);
        case ESP32BLEComboJISMode::GAMEPAD_ONLY: return static_cast<uint16_t>(ESP32BLEComboJISAppearance::GAMEPAD);
        case ESP32BLEComboJISMode::KEYBOARD_MOUSE_GAMEPAD: return static_cast<uint16_t>(ESP32BLEComboJISAppearance::GENERIC_HID_APPEARANCE);
        default: return static_cast<uint16_t>(ESP32BLEComboJISAppearance::GENERIC_HID_APPEARANCE);
    }
}

bool ESP32BLEComboJIS::initAddressOverride() {
...
bool ESP32BLEComboJIS::begin(const ESP32BLEComboJISConfig& config) {
    ...
    m_server->setCallbacks(new ESP32BLEComboJISServerCallbacks(this));
    ...
    switch (m_mode) {
        case ESP32BLEComboJISMode::KEYBOARD_ONLY: map = keyboardReportMap(mapLen); break;
        case ESP32BLEComboJISMode::MOUSE_ONLY: map = mouseReportMap(mapLen); break;
        case ESP32BLEComboJISMode::GAMEPAD_ONLY: map = gamepadReportMap(mapLen); break;
        case ESP32BLEComboJISMode::KEYBOARD_MOUSE_GAMEPAD: map = kComboAllMap; mapLen = sizeof(kComboAllMap); break;
...
void ESP32BLEComboJIS::startAdvertising() {
...
bool ESP32BLEComboJIS::isConnected() const { return m_connected; }
ESP32BLEComboJISMode ESP32BLEComboJIS::getMode() const { return m_mode; }
void ESP32BLEComboJIS::setBatteryLevel(uint8_t level) { m_config.batteryLevel = level; if (m_hid) m_hid->setBatteryLevel(level); }
void ESP32BLEComboJIS::setDeviceName(const String& name) { m_config.deviceName = name; }
void ESP32BLEComboJIS::setManufacturer(const String& manufacturer) { m_config.manufacturer = manufacturer; if (m_hid) m_hid->setManufacturer(manufacturer.c_str()); }
void ESP32BLEComboJIS::setKeyPressDelay(uint16_t ms) { m_keyPressDelayMs = ms; m_config.keyPressDelayMs = ms; }
void ESP32BLEComboJIS::setKeyReleaseDelay(uint16_t ms) { m_keyReleaseDelayMs = ms; m_config.keyReleaseDelayMs = ms; }
void ESP32BLEComboJIS::setKeyIntervalDelay(uint16_t ms) { m_keyIntervalDelayMs = ms; m_config.keyIntervalDelayMs = ms; }

void ESP32BLEComboJIS::sendKeyboardReport() {
...
void ESP32BLEComboJIS::sendMouseReport(int8_t x, int8_t y, int8_t wheel, uint8_t buttons) {
...
void ESP32BLEComboJIS::sendGamepadReport() {
...
bool ESP32BLEComboJIS::write(uint8_t c) {
...
bool ESP32BLEComboJIS::print(const char* text) {
...
bool ESP32BLEComboJIS::print(const String& text) { return print(text.c_str()); }

bool ESP32BLEComboJIS::press(uint8_t k) {
    if (!keyboardEnabled()) return false;
    if (k >= 136) k -= 136;
...
bool ESP32BLEComboJIS::release(uint8_t k) {
    if (!keyboardEnabled()) return false;
    if (k >= 136) k -= 136;
    else if (k >= 128) { m_keyReport.modifiers &= ~(1 << (k - 128)); k = 0; }
    else {
...
bool ESP32BLEComboJIS::releaseAll() {
    if (!keyboardEnabled()) return false;
    memset(&m_keyReport, 0, sizeof(m_keyReport));
    sendKeyboardReport();
    return true;
...
bool ESP32BLEComboJIS::mouseMove(int x, int y) {
...
bool ESP32BLEComboJIS::mouseScroll(int amount) {
    if (!mouseEnabled()) return false;
    while (amount != 0) {
...
bool ESP32BLEComboJIS::mousePress(uint8_t buttons) {
    if (!mouseEnabled()) return false;
...
bool ESP32BLEComboJIS::mouseRelease(uint8_t buttons) {
    if (!mouseEnabled()) return false;
    m_mouseReport.buttons &= ~buttons;
    sendMouseReport(0, 0, 0, m_mouseReport.buttons);
    return true;
...
bool ESP32BLEComboJIS::gamepadRelease(uint8_t buttonIndex) {
    if (!gamepadEnabled() || buttonIndex < 1 || buttonIndex > 16) return false;
    m_gamepadReport.buttons &= ~(1u << (buttonIndex - 1)); sendGamepadReport(); return true;
}

bool ESP32BLEComboJIS::gamepadSetAxes(int16_t x, int16_t y, int16_t z, int16_t rz) {
    if (!gamepadEnabled()) return false;
    m_gamepadReport.x = x; m_gamepadReport.y = y; m_gamepadReport.z = z; m_gamepadReport.rz = rz; sendGamepadReport(); return true;
}

bool ESP32BLEComboJIS::gamepadSetHat(uint8_t hat) {
    if (!gamepadEnabled()) return false;
    m_gamepadReport.hat = hat; sendGamepadReport(); return true;
}

bool ESP32BLEComboJIS::gamepadReleaseAllButtons() {
    if (!gamepadEnabled()) return false;
    m_gamepadReport.buttons = 0; sendGamepadReport(); return true;
}

bool ESP32BLEComboJIS::gamepadSetTriggers(int16_t leftTrigger, int16_t rightTrigger) {
    if (!gamepadEnabled()) return false;
...
bool ESP32BLEComboJIS::gamepadSetLeftStick(int16_t x, int16_t y) {
    if (!gamepadEnabled()) return false;
    m_gamepadReport.x = x;
    m_gamepadReport.y = y;
    sendGamepadReport();
...
bool ESP32BLEComboJIS::gamepadSetRightStick(int16_t x, int16_t y) {
    if (!gamepadEnabled()) return false;
