...
bool handleCommand(const String& raw) {
  ...
  if (input.startsWith("/click ")) {
    ...
    which.toLowerCase();
    if (which == "left") ble.mouseClick(ESP32BLEComboJIS::MOUSE_LEFT);
    else if (which == "right") ble.mouseClick(ESP32BLEComboJIS::MOUSE_RIGHT);
    else if (which == "middle") ble.mouseClick(ESP32BLEComboJIS::MOUSE_MIDDLE);
...
void setup() {
  ...
  cfg.batteryLevel = 70;
  cfg.appearance = ESP32BLEComboJISAppearance::AUTO;
