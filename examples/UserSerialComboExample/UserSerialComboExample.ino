...
bool handleCommand(String input) {
    ...
    if (input.startsWith("/click ")) {
        ...
        if (which == "left") {
            ble.mouseClick(ESP32BLEComboJIS::MOUSE_LEFT);
            Serial.println("Clicked: left");
        } else if (which == "right") {
            ble.mouseClick(ESP32BLEComboJIS::MOUSE_RIGHT);
            Serial.println("Clicked: right");
        } else if (which == "middle") {
            ble.mouseClick(ESP32BLEComboJIS::MOUSE_MIDDLE);
    ...
    if (input == "/release") {
        ble.mouseRelease(
            ESP32BLEComboJIS::MOUSE_LEFT |
            ESP32BLEComboJIS::MOUSE_RIGHT |
            ESP32BLEComboJIS::MOUSE_MIDDLE
        );
...
void setup() {
    ...
    cfg.batteryLevel = 70;
    cfg.appearance = ESP32BLEComboJISAppearance::KEYBOARD;
